...
if ($null -eq $Meta.PSObject.Properties["geometry_version"]) {
    throw "V12 refuses legacy fixture. Run the V11 geometry-correct preparation first."
}

if ([int]$Meta.geometry_version -lt 2) {
    throw "V12 refuses legacy fixture. Run the V11 geometry-correct preparation first."
}
...
if ($Rows -ne 9216 -or $Cols -ne 2560) {
    Write-Host "Warning: expected current Qwen tensor 9216 x 2560; found $Rows x $Cols" -ForegroundColor Yellow
}
...
Write-Host "============================================================"
Write-Host ("Compiler        : {0}" -f $Gpp.Source)
Write-Host ("Matrix          : {0} rows x {1} cols" -f $Rows, $Cols)
Write-Host ("CPU affinity    : {0}  (-1 means scheduler)" -f $Cpu)
Write-Host ("Eviction buffer : {0} MiB" -f $EvictMiB)
...
function Build-And-Run {
    ...
    Write-Host (" BUILD/RUN {0}" -f $Name) -ForegroundColor Cyan
    Write-Host "============================================================"
...
Write-Host "  paired_speedup less than 1.00 means llama-style AVX2 wins"
Write-Host ""
Write-Host "Send back the HOT and EVICTED result lines from both builds."
